...
1. Generate one or more Claude Code OAuth tokens (Claude Pro/Max/Teams
   ...
   - **Chat + 5h/7d usage-limit tracking** (recommended): double-click
     `scripts/get_full_scope_oauth_token.bat` (no runtime install
     needed — it runs the PowerShell script bundled with every
     supported Windows version). Follow the printed instructions. This
...
`scripts/get_full_scope_oauth_token.bat` (see Setup) to register a token
